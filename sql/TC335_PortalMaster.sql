-- =============================================================================
--  Portal Master  (faction-aware city teleporter)
...
--
--  A SmartAI gossip teleporter whose destinations are gated by faction via the
--  conditions table. Uses SmartAI (no AC core script), so it ports cleanly.
--  Port fixes: InhabitType removed / AiName->AIName; creature_template_addon
--  bytes1/bytes2 dropped; spawndist->wander_distance; gossip_menu(_option) and
...
(@TEXT_ID+5, "$BWhich Moo Portal would you like to take?$B"),
(@TEXT_ID+4, "$BWhere would you like to be ported?$B"),
(@TEXT_ID+3, "$BBe careful with choosing raids, I won't be there if you wipe!$B"),
(@TEXT_ID+2, "$BUp for some dungeon exploring?$B"),
(@TEXT_ID+1, "$B For The Alliance!$B"),
(@TEXT_ID,  "$B For the Horde!$B");
